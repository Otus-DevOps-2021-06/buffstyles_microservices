...
## Домашнее задание №26 - kubernetes-4.

### Основное задание:

 - Установлен Helm v2, в кластере заупщен Tiller сервер.
 - Создан чарт для `ui` компонента приложения.
 - Чарт `ui` шаблонизирован.
 - Аналогично созданы шаблонизированные чарты `comment` и `post` компонент.
 - В чарты добавлены хелперы `_helpers.tpl` для реализации более сложной логики. В хелперы добавлены функция `fullname`.
 - Создан единый чарт приложения `reddit`, который объединяет наши компоненты. В зависимости добавлены необходимые компоненты.
 - Установлен Helm2 tiller plugin. Выполнена установка приложения в новый неймспейс с помощью плагина.
 - Установлен Helm v3. Выполнена установка приложения в новый неймспейс.
 - Поднят кластер `Kubernetes`. Установлен `Gitlab 14.0.4` из репозитория Gitlab. Gitlab-Omnibus находится в статусе DEPRECATED. Каталог gitlab-omnibus создан ради прохождения авто-тестов. Основной чарт в каталоге `gitlab`.
 - Создана группа проектов. В группе созданы проекты `post`, `reddit-deploy`, `comment` и `ui`.
 - Локально создан репозиторий `Gitlab_ci`. В соответствующие проекты перенесены исходные коды компонент.
 - Для трех компонент созданы файлы `.gitlab-ci.yml`.
 - Изменен конфиг `ingress` для сервиса `ui`.
 - В `.gitlab-ci.yml` добавлена стадия `review` для запуска отдельного окружения по коммиту в feature-branch. Добавлена функция `deploy()` которая загружает Chart из репозитория
reddit-deploy и делает релиз в неймспейсе review с образом приложения, собранным на стадии build. Добавлена стадия `cleanup` для удаления временных окружений.
 - Итоговый файл `.gitlab-ci.yml` для `ui` скопирован в репозитории `post` и `comment`. Проверено аналогичное создание динамического окружения для этих компонент.
 - Создан файл `.gitlab-ci.yml` для репозитория `reddit-deploy`.
 - Для трех компонент переделан пайплайн, логика вынесена из `auto_devops` в соответствии с синтаксисом Gitlab.
 - Пайплайн сервиса `comment` изменен на использование Helm v2 tiller plugin.
 - Пайплайн сервиса `comment` изменен на использование Helm v3.
 - Пайплайн для `reddit-deploy` изменен аналогичным образом для избавления от `auto_devops`.
 - Файлы `.gitlab-ci.yml` , полученные в ходе работы, помещены в папку с исходниками для каждой компоненты приложения.
 - Файл `.gitlab-ci.yml` для `reddit-deploy` помещен в Сharts.
 - Все изменения, которые были внесены в Chart’ы - перенесены в папку Сharts, созданную вначале.

### Дополнительное задание 1:

 - Реализованы `Multi-project pipelines`. Пайплайны сборки образов приложений и пайплайн деплоя на staging и production связаны так, чтобы после релиза образа из ветки мастер запускался деплой уже новой версии приложения на production.


## Домашнее задание №27 - kubernetes-5.

### Основное задание:

 - Установлен nginx-ingress.
 - Скачан и установлен чарт [Prometheus](kubernetes/Charts/prometheus).
 - Включен kube-state-metrics в чарте Prometheus.
 - Включен node-exporter в чарте Prometheus.
 - Запущен чарт reddit. Конфиг Prometheus изменен. Добавлен ServiceDiscovery для обнаружения reddit.
 - Конфиг Prometheus для `reddit-endpoints` разбит на компоненты приложения `post-endpoints`, `comment-endpoints` и `ui-endpoints`.
 - Установлена Grafana.
 ```
helm upgrade --install grafana grafana/grafana --set "ingress.enabled=true" --set "service.type=NodePort" --set "ingress.hosts={grafana.<ingress_external_ip>.nip.io}"
 ```
 - Добавлены дашборды, отражающие параметры работы приложения. Дашборды шаблонизированы для работы с
несколькими окружениями. Шаблонизированы запросы к Prometheus.


### Дополнительное задание 1:

 - Запущен Alertmanager в конфиге Prometheus ([prometheus/custom_values.yaml](kubernetes/Charts/prometheus/custom_values.yaml)). Настроены правила за доступностью api-сервера и хостов k8s. Добавлен алертинг в Slack.

### Дополнительное задание 2:

 - Установлен Prometheus Operator с помощью чарта `Charts/kube-prometheus-stack`. Настроен мониторинг post endpoints. Используемый [ServiceMonitor](kubernetes/Charts/servicemonitor.yaml) манифест.

### Дополнительное задание 3:

 - Создан Helm-чарт для установки стека EFK.
